...
Choose by population: per-client `interest` (above) for a small-to-mid room where each client's view is individual; `interest: { cells: true }` for an arena of thousands. Both leave the `interest`-off broadcast path byte-identical. Requires `svelte-adapter-uws >= 0.6.0-next.46` (the cell-snapshot codec). `hitTest` composes with `cells`: the shooter's candidate set is then its cell subscription - the exact set replicated to it - instead of the per-client relevancy membership, with the same rewind-instant radius gate on top. Cells mode is single-instance today (cross-node cell delivery rides the forthcoming cluster mirror).
