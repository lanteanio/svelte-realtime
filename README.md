...
**In a cluster**, redaction runs on the instance that publishes. A dynamic (factory) topic can be published from an instance that never subscribed to that resolved topic (a peer holds the subscriber), so the framework also registers the stream's redactor keyed by the factory's derived topic *skeleton* - `(ctx, roomId) => 'chat:' + roomId` derives `chat:{arg}` - and applies it to any resolved instance send-side, before the wire and the cluster relay. Full cluster-transient coverage needs a topic the framework can reduce to a safe skeleton: a **leading literal namespace** plus **single-segment** variables (each spanning one `/`- or `:`-delimited segment). Two shapes fall short, and they fail differently:

- **Warned at declaration (no coverage at all).** A topic read from `ctx` (`(ctx, room) => ctx.user.org + ':' + room`), a variable-first topic (`(ctx, id, kind) => id + ':' + kind`), or a constant cannot be reduced to a usable skeleton, so the stream has no cluster-transient pattern and a **dev warning fires at declaration** naming the derived skeleton. Fix the topic shape, and keep per-tenant isolation on the framework's [tenant scoping](#multi-tenancy-livetenant) rather than concatenating a server-trusted `ctx` value into the topic string (moving it to a client-supplied argument would let a caller spoof the tenant).
- **Silent runtime residual (no warning).** A factory with a perfectly good skeleton (`chat:{roomId}`) whose *resolved* argument itself contains a `/` or `:` (a composite id like `acme:general`, yielding `chat:acme:general`) slips past the single-segment matcher at run time. No declaration warning can fire (the skeleton is indistinguishable from a safe one), so that specific never-subscribed, cross-instance publish relays raw until a local subscriber resolves the exact topic. Keep resolved ids to a single segment.

In both cases, single-instance deployments and any topic a local subscriber has already resolved are always redacted; only a cross-instance publish to a topic *this* instance never subscribed to is exposed. Prefer a `chat:${roomId}` shape (leading namespace, one segment) for a clustered `piiRedact` stream.
...
//         idempotency, smooth, webhookDeadLetter, aggregateCohorts, durable,
//         crdtDocs?, ownerSuccessions? } }
```
...
The store's `purgeUser` may return a total (number), a per-store breakdown (object of counts), or the owner-succession envelope `{ rowsAffected, ownerSuccessions }`. The envelope's `ownerSuccessions` array (`{ topic, owner, reason }` per room, the room's wire data topic) carries the `:owner` changes the store's cluster-wide owner eviction produced - the rooms the erased user owned that the forgetting instance could NOT resolve locally (the user's room refs live on another instance, or the identity is still connected elsewhere so the local leave never reaches the cluster-wide zero). `live.forget` publishes each on the room's `:owner` topic through the replay buffer, so live subscribers on every replica see the successor and a resuming client gap-fills it instead of the erased owner; the count lands in `surfaces.ownerSuccessions`. Without the envelope (a plain count store) behavior is unchanged - succession still resolves locally for rooms this instance holds. The room-level `onOwnerChange` server hook does not fire for these fan-in successions (it is bound to a room's local join state, which the forgetting instance need not hold); the wire event and replay buffer are the erasure-critical halves.
...
Resolution: `keyFrom(ctx, ...args)` if defined, otherwise the client envelope's `idempotencyKey`, otherwise the wrapper is a no-op. Only successful results cache; a handler that throws BEFORE commit aborts the slot so the next caller re-runs. A failure DURING commit is different: the handler already ran its effect, so the slot is left intact and the call returns `LiveError('IDEMPOTENCY_COMMIT_UNKNOWN')` rather than aborting - aborting would delete a record the durable store may have applied (a lost ack) and let a retry re-run the handler and double-apply the effect, so the caller should reconcile instead of blindly retrying. Reusing a key with a **different request payload** throws `LiveError('IDEMPOTENCY_KEY_REUSED')` instead of returning the first call's result - a genuine idempotent retry must carry the same body, so the framework fingerprints the request args (canonical, key-order-insensitive) and rejects a mismatch rather than silently answering with the wrong cached value. Default store is in-process and bounded; for multi-instance deployments pass `store: createIdempotencyStore(redis)` from `svelte-adapter-uws-extensions`.

The cache slot is automatically isolated by RPC path and by tenant, so the same key under different RPCs or tenants never collides. When the key comes from the **client envelope** (no `keyFrom`), it is additionally isolated by the caller's authenticated identity - so one user can never receive another user's cached result by sending the same key, even though the client controls the envelope value. A `keyFrom` key is app-owned: include `ctx.user` in the returned string for per-user idempotency, or omit it for a deliberately shared slot (e.g. an org-wide daily job). Anonymous callers are not identity-scoped, so use a client-random key (a UUID per intent, as above) for them.
...
- **Selections** - `room.selections` is a `{ userKey: range }` map of remote selection ranges (self excluded). In the default `selections: 'offset'` mode you publish a raw range with `room.setSelection({ start, end, nodePath })` (pass `null` to clear); a selection is stamped on the caller's roster entry, so a late joiner loads the current selections from the roster snapshot. Raw offsets are correct only while the document does not change underneath them - the moment another user inserts before a selection, the offsets point at the wrong text. For collaborative editors, declare `selections: 'crdt'` and bind the room to the document the selections index into:

  ```svelte
  <script>
    const room = board.room(boardId);
    const doc  = board.doc?.(boardId) ?? editorDoc;  // the live.doc for this document
    room.bindDoc(doc);                                // once, client-side

    // publish a selection against a named text container - still plain offsets
    function onSelect(range) {
      room.setSelection({ field: 'body', start: range.from, end: range.to });
    }
  </script>

  {#each Object.entries(room.selections) as [user, sel]}
    <Highlight field={sel.field} start={sel.start} end={sel.end} />  <!-- survives others' edits -->
  {/each}
  ```

  In `'crdt'` mode `setSelection` encodes the range into a position anchor bound to the document, and `room.selections` resolves every collaborator's anchor back to current `{ field, start, end }` offsets - reactively, so a rendered highlight re-resolves onto the right characters as the document is edited. An edit exactly at a selection edge stays outside it, one strictly inside extends it, and a selection whose text is deleted collapses to a caret. The value on the wire stays opaque (the server never interprets it), and an anchor that cannot resolve is dropped rather than rendered wrong. Requires `svelte-adapter-uws >= 0.6.0-next.45`; a crdt selection without `bindDoc` is dropped with a dev warning.
...
- **The remote world has its own health signal.** `view.overflowed` watches the *local* command window; a blackout where the server stops sending *remote* frames on a still-open socket is invisible to it. `view.stalled` covers that gap - true once no authority frame has arrived for the channel's stall window (`stallMs`, default 1000) while remote entities are tracked, clearing when frames resume, and folding into the same `health` `'degraded'` state so one banner covers both. Per entity, `view.freshness(key)` reports `'live'`, `'coasting'` (dead-reckoned within the extrapolation cap), or `'stale'` (frozen on stale data), so a renderer can dim a coasting ghost: `<Box ... class:stale={view.freshness(key) === 'stale'} />`. The same tag also rides each `remote` state under the exported `SMOOTH_FRESHNESS` Symbol. Needs svelte-adapter-uws >= 0.6.0-next.59.
...
**Cell-topic interest (population scale):** per-client culling computes each subscriber's visible set on the server and sends it *per subscriber*, so every client's frame differs and the server encodes once per connection - fine at hundreds of subscribers, a wall at thousands. For a high-population arena, set `interest: { cells: true }` and area-of-interest becomes SUBSCRIPTION instead of server-side filtering:

```js
export const mega = live.smooth({
  topic: (ctx, arenaId) => `arena:${arenaId}`,
  apply,
  initial: (key) => spawn(key),
  interest: {
    cells: true,                            // grid the world into cell topics
    radius: 1200,                           // how far a subscriber sees (its cell block)
    cell: 256,                              // grid cell size in position units (default 256)
    position: (s) => ({ x: s.x, y: s.y })   // where an entity is; null = always-visible
  }
});
```

The world is gridded into cells; each cell is a topic. The server subscribes each socket to the cell block covering its view - from the subscriber's own entity position by default, or a reported `view.reportCenter(x, y)` for a spectator/free-cam - and (un)subscribes as it crosses cell boundaries (with a one-cell hysteresis margin so hovering on an edge does not thrash). Each changed entity is published to its cell's topic, and because that snapshot is identical to every subscriber of the cell it fans out natively (one encode per cell, uWS does the sends) instead of a per-connection walk. Egress is then **O(cells), not O(subscribers)** - CPU steps aside and bandwidth becomes the binding constraint, which is the goal. A subscriber's own entity still rides the self channel (predicted + reconciled); the client renders it from its own prediction and drops its own key from the cell broadcasts, so there is no ghost twin.

Choose by population: per-client `interest` (above) for a small-to-mid room where each client's view is individual; `interest: { cells: true }` for an arena of thousands. Both leave the `interest`-off broadcast path byte-identical. Requires `svelte-adapter-uws >= 0.6.0-next.46` (the cell-snapshot codec). `hitTest` composes with `cells`: the shooter's candidate set is then its cell subscription - the exact set replicated to it - instead of the per-client relevancy membership, with the same rewind-instant radius gate on top.

Cells mode works single-instance and across the cluster. The topic's owner relays each cell frame - with its cell topic - over the smooth coordinator (the same in-order channel and per-owner sequence the base broadcasts ride), and every other instance republishes it natively to its own subscribed sockets through its own codec: no frame bytes or wire ids ever cross the bus. Each instance drives its OWN sockets' cell subscriptions - a joiner's block is placed at sync (on the owner from its ensured entity; on a non-owner from its own entry in the owner's reply), follows the subscriber's own entity via its acknowledgements and relayed updates, and honors a reported `view.reportCenter(x, y)` locally - and the join roster stays cell-scoped on every path. Two residuals to know: a command-less remote entity that `onMissing` teleports beyond its subscriber's block breaks the own-entity follow until a center report or resync, and a shot forwarded from a non-owning instance gates on the rewound-position radius rather than receipt-time cell membership (the same polarity per-client interest has on a cluster - the gate stays server-side either way).

**Hit detection (lag-compensated shooting)** is the latency problem the [`live.action` lag compensation](#lag-compensation) section solves, applied to smoothed entities: by the time a player's shot reaches the server every target has moved on, so a hit that landed on the shooter's screen misses on the server. Declare `hitTest` and the server resolves each shot against the world *as the shooter saw it* - it rewinds every candidate entity to the instant the shooter rendered it, tests the shot against those historical positions, and applies the result authoritatively, with no bit-exact cross-host determinism contract. It builds on `interest` - the shooter's replicated set is the security gate, so `hitTest` requires `interest` (either mode: the per-client relevancy membership, or the shooter's cell subscription under `cells: true`):
...
export const arena = live.smooth({
  ...
  apply,
  initial: (key) => spawn(key),
  interest: { radius: 1200, position: (s) => ({ x: s.x, y: s.y }) },
  hitTest: {
    shot: {
      type: 'ray',
      origin: (cmd, state) => ({ x: state.x, y: state.y }),
      dir: (cmd) => cmd.angle,                 // radians, or a { x, y } vector
      maxDist: 900
    },
    hitbox: { shape: 'circle', radius: 24 },   // or { shape: 'aabb', w, h }
    onHit: (ctx, target, info) => {
      ctx.applyTo(target.key, { type: 'damage', amount: 25 });  // authoritative
      ctx.emitEvent('hit', { by: ctx.identity, at: info.point });
      return { stop: true };                   // stop at the first body hit
    }
  }
});
```
...
<script>
  ...
  function onfire(angle) {
    view.shoot({ angle });   // fire-and-forget; the framework stamps the timing
  }
</script>
```
...
- **The shooter proposes *when*, the server bounds *how far*.** The client stamps each shot with the synced-clock instant it rendered the scene (the same axis `view.now()` reports - you pass only your gameplay `cmd`, the framework adds the timing). The server rewinds to that instant but clamps the window to a reach it measures itself: `reach = measured uplink + interpolation delay`, capped at `maxRewindMs`. Both legs are server-measured - the uplink from the shot's acknowledgement round trips, the interpolation delay from how often the server sends that shooter frames (the same cadence the client uses to set its own render delay) - so a client can inflate its apparent latency only by genuinely lagging (which costs it responsiveness), never fake a lower one to reach further back. A low-latency shooter stays tight and cannot borrow a laggy player's budget; a sparsely-served one (its neighbours throttled by level-of-detail, or sitting in a quiet region) gets the wider reach it genuinely needs instead of clamping short.
- **The candidate set is the shooter's replicated set, evaluated at the rewind instant.** You cannot rewind or hit an entity that was never replicated to the shooter, so the candidate set is exactly the `interest` membership - the per-client relevancy set, or under `cells: true` the entities in the shooter's subscribed cells - and the client supplies none of it: it cannot widen the set. Membership is measured geometrically at the rewound instant, not at receipt: a target the shooter had on screen when it fired is a valid hit even if it drifted out of range while the shot was in flight, and one that drifted in only after the shot was fired is not.
- **Narrowphase is yours to pick.** Give a declarative `hitbox` (`{ shape:'circle', radius }` or `{ shape:'aabb', w, h }`) and the framework runs the ray test, or supply `resolve(shot, target, ctx)` for custom geometry. Either way the framework owns the rewind and the candidate gate, and the rewound `target.state` carries the time-correct stance - a crouch or a raised shield that was up when the shooter fired is honored.
- **`onHit(ctx, target, info)` is the consequence, run nearest-first.** `target` is `{ key, pos, state }` (rewound) and `info` is `{ dist, point, fraction, rewindAt, fallback }` (`fallback: true` means the rewind could not be served and current state was used). Inside it, `ctx.applyTo(victimKey, cmd)` applies an authoritative cross-entity mutation - a hit dropping the victim's health, a server-initiated update the victim still receives, so its own prediction is undisturbed - and `ctx.emitEvent(type, data, opts?)` signals the hit on the `view.onEvent` channel (the shooter's hit marker and the victim alike see it; `opts.key` overrides the event key, default the shooter). The ray penetrates by default - `onHit` fires for every body it passes through, nearest first - so return `{ stop: true }` for the common hitscan-blocks-on-the-first-body case. `onHit` may be async.
- **`view.shoot(cmd)` is fire-and-forget and never predicted.** A shot owns no entity state to predict, so it bypasses the prediction ring entirely; its outcome arrives as a discrete `view.onEvent`, not a reconciliation, and a lost shot is yours to retransmit (the framework never does). Inert on a topic declared without `hitTest`.
- **Safe by construction.** The rewind never extrapolates; a candidate whose path straddles a death/respawn gap misses rather than resolving against a phantom position between the corpse and the spawn point; and a hostile, out-of-window, or replayed render-time (a captured shot resent to re-resolve an old enemy lineup) fails safe to the current position - every ambiguity favors the defender.

The one knob to understand is **`maxRewindMs`** (default `100`): the furthest back any shot may rewind, i.e. the worst-case "shot around the corner" a defender can eat. It is the security-critical setting - a latency-faker can at most look like a real player at this latency. The default is competitive and defender-friendly for a ~60Hz topic (it fully compensates a good connection - uplink plus interpolation - while bounding the peeker's advantage to about one body-width at speed); raise it to favor the shooter or support a high-ping community (the casual value is `1000`), but keep it at or above one interpolation delay (~`2 x tickMs`) or honest players whose interp buffer alone exceeds it clamp every shot.

Other knobs (all optional): `teleportThreshold` (position units - abort the rewind for a candidate whose path jumps more than this across the rewind bracket; off by default, since the always-on gap guard already covers respawns), `broadphase: { maxDist, cone }` (tune the distance / cone-of-fire cull; `cone` is a cosine in `[-1, 1]`), `position` (the function the rewind and candidate gate read; defaults to `interest.position`), and `detectionHook`. The interpolation leg of the reach is **server-measured and never client-trusted** by design - that is the trust boundary the whole rewind window rests on.

> **Coordinate spaces.** `hitTest.position` defaults to `interest.position` so the rewind gate and the relevancy set share one space. Supplying a *custom* `position` in a different coordinate space makes the framework skip the rewind-instant candidate gate (mixing spaces would be unsound) and fall back to receipt-time membership - still interest-space correct, just not rewind-instant precise. Keep `hitTest.position` and `interest.position` in the same coordinate space unless you have a specific reason not to.

**`detectionHook`** (opt-in, off by default) surfaces a per-shot latency picture to an anti-cheat callback: `detectionHook({ identity, minUplink, maxUplink, reach, interpDelay, divergence })` runs once per resolved shot. The discriminating lag-switch tell is `divergence` - the spread between the un-inflatable measured floor (`minUplink`) and the reach-driving recent maximum (`maxUplink`) - plus an abrupt floor jump you derive from the `minUplink` series, not the raw window-clamp rate (honest jittery, mobile, or reconnecting players clamp routinely and must not be punished). The framework only emits the signal; the action - flag, kick, shadow - belongs to your module, and a hook that throws never affects the shot.

**`defenderAllowance`** (opt-in, off by default) leans further toward the holder than the flat `maxRewindMs` cap does, but only for the case that stings most - getting shot after reaching cover. Because the framework has positions, not occlusion, you supply `defenderAllowance: { exposure(shooterState, targetState), allowanceMs }`: `exposure` is your wall / cover test (do these two see each other), and a candidate visible to the shooter at the rewind instant but occluded by `min(now, rewindAt + allowanceMs)` - it reached cover within the window - is dropped from that shot. It is **strictly subtractive**: it can only ever turn a hit into a miss, never the reverse, so it can never help a shooter regardless of how your occlusion relates to the shot geometry; a throwing `exposure` fails safe to no grace; and `allowanceMs` only ever relaxes `maxRewindMs` toward the present, never past it. Leave it off to favor the shooter within the cap (the default); turn it on, tuned to your arena, to give a holder who reached cover the benefit of the doubt.
