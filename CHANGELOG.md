...
- **Vite plugin: self-sustaining HMR loop through `$types.d.ts` regeneration - full-reload storm for every connected browser (reported in #9).** `handleHotUpdate` filtered only on the live directory, with none of the `.d.ts` / `.test.*` exclusions the add/unlink watcher already applied, so the plugin's own generated `src/live/$types.d.ts` fed back into its own handler. Each pass rewrote the file unconditionally (fresh mtime even for identical content), which scheduled the next change event: once anything touched `$types.d.ts` after the watcher was armed - most commonly a second Vite instance on the same checkout running its `buildStart()` write, e.g. a Playwright test server next to the dev server - the loop sustained itself forever, spamming `Hot-reloaded: src/live/$types.d` every 1-3 s, running a full SSR registry reload per tick, and full-reloading every connected client (the file is not in the module graph, so Vite fell back to `full-reload`). The loop survived the second instance exiting and never healed without a restart. Two-part fix: `handleHotUpdate` now returns `[]` for `.d.ts` files (handled, no affected modules - suppresses the fallback full reload) and skips test files like the watcher does; and `_writeTypeDeclarations` compares the existing file content before writing, so an unchanged regeneration is a no-op with no mtime churn - which also removes the cross-process ignition, since a second instance's boot write of identical content no longer produces a change event at all. Regression tests pin all three behaviors: `$types.d.ts` events are ignored without a rewrite or registry reload, test-file events are ignored, and an unchanged `$types.d.ts` keeps its mtime while a real content change is still written.
...
- **Server-initiated push via `live.push()` and client-side `onPush()`.** New primitive on the `live` function namespace for sending a request to a connected user and awaiting their reply. Routes through a per-instance userId -> WebSocket registry maintained by a small pair of hooks.
  ...
  Single-instance routing only in this release: a user's connection must live on the same server process that calls `live.push`. Cluster-wide push (any instance routing to any user's WebSocket) requires the connection-registry primitive in the extensions package.
