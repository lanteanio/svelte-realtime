...
## [0.6.0-next.90] - 2026-07-30

### Breaking Changes

- **Outbound webhook signatures are now timestamped: receivers must verify `HMAC_SHA256(secret, timestamp + '.' + rawBody)`.** The delivery engine lives in `svelte-adapter-uws/plugins/webhooks`, whose new release signs `<unix-seconds>.<rawBody>` and ships the timestamp alongside as `x-webhook-timestamp`, so a captured delivery can no longer be replayed indefinitely - the receiver bounds freshness against its own clock. A receiver still verifying the old body-only material rejects every delivery after this upgrade; the old signature is deliberately not emitted alongside the new one, because a receiver accepting either form keeps the replay hole open. Verify with the plugin's `verifyWebhookSignature`, or follow the four-step receiver contract documented on `secret`. The `svelte-adapter-uws` peer floor moves to `^0.6.0-next.87` accordingly, and the swarm/test toolchain tracks `svelte-adapter-uws-extensions` `^0.6.0-next.63`, which also carries the `createForgetStore` owner-succession envelope the next.89 forget fixes consume. The `faultMode` rename below lands in lockstep across all three packages.

- **The sim swarm's fault-enablement knob is renamed `faultMode`.** On `runLiveSimSwarm` and the smooth swarm, `buggify` becomes `faultMode`, `buggifyProbability` becomes `faultProbability`, and the per-run flag and summary counter `buggified` become `faulted`. Mirrors the same rename in `svelte-adapter-uws` and `svelte-adapter-uws-extensions`, whose swarm contract this tracks. The old name was a term coined by another project and explained nothing on its own; `faultMode` shares a stem with the `faultProfile` and `faultProbability` knobs beside it. The committed golden corpora were re-blessed, since the per-seed fault coin derives from a seed-suffixed string that changed with the rename; regenerate a local corpus with `npm run sim:golden -- --update`.

### Security

- **`guard({ authenticated: true })` now rejects anonymous connections.** It tested `ctx.user != null`, but `svelte-adapter-uws` always upgrades a socket with a userData object (an anonymous one arrives as `{ remoteAddress }`), so the gate never fired on a real connection and any handler whose only authentication was this guard was reachable unauthenticated. The gate now keys off an authenticated IDENTITY - `ctx.user.id` (or `user_id` / `userId`) - at every classification site including `live.scoped()`. A present-but-empty id (`{ id: session?.userId ?? '' }`, a common no-session sentinel) no longer authenticates; `id: 0` still does, and each alias is probed independently so an empty `id` cannot mask a valid `user_id`. Object ids keep working (Mongo `ObjectId`, Prisma `Decimal`, a `Buffer`/`Uint8Array` uuid, which is read as bytes rather than UTF-8-decoded), but a value whose only stringification is a generic `[object Promise]` / `[object Map]` tag is rejected - accepting one would collapse every connection onto a single identity, which is what a forgotten `await` on `{ id: db.getUserId() }` produces.

- **`live.access.owner()`, `.team()`, `.org()` and `.user()` now require a value that identifies somebody.** They tested `!= null`, which admits `''` - the standard no-session sentinel (`{ id: session?.userId ?? '' }`) - and also a boolean or a plain object. So the shipped access predicates carried the same anonymous-connection hole as the authenticated guard above, on the surface whose own documentation offers it as the way to close authorization-bypass holes around per-org streams and RPCs. On the comparing predicates it was worse: the caller supplies the value compared against the expected one, so `''` on both sides matched exactly and any client could satisfy `org()`/`user()` against an unauthenticated connection. All four now apply the same VALUE test as the guard (note it is the same value rule, not the same field probe: the guard tries `id`/`user_id`/`userId` in turn, while `owner(field)` reads the one field you name). `0` remains a valid id; object ids are accepted by `owner()`/`team()`, which only test the field. `org()`/`user()` still compare the **raw** value with `===`, so a numeric org id continues to require a numeric argument - and by the same token an `ObjectId`/`Buffer` there can never equal a wire value, so normalize via `from` for those shapes. **Breaking** only for code that relied on a *non-identifying* value passing - notably `live.access.owner('someBooleanFlag')`, which used to pass on `true`.

- **`live.access.role()` no longer matches inherited `Object.prototype` keys, and `live.access.all()` no longer fails open.** `role()` tested `role in map`, which walks the prototype chain, so `ctx.user.role` of `'constructor'` or `'toString'` matched a map that never declared it; the looked-up "rule" was then an `Object.prototype` member, and because the gate is a truthy test, `map['constructor']` (the `Object` constructor, returning a truthy object) and `map['toString']` (returning `'[object Undefined]'`) both **admitted** the connection. Any app that maps a client-influenced role claim onto `ctx.user.role` was one such string away from passing a role gate it never declared. Lookup is now own-keys-only, a non-string role is rejected, and a mapped value that is neither `true` nor a function denies instead of being called - the documented `{ viewer: false }` shape previously threw a `TypeError` that surfaced as `INTERNAL_ERROR`, turning an ordinary denial into a reported server fault. Separately, `live.access.all()` with **no** predicates returned `true`, so a rule list that resolved empty (config-driven, env-gated, or a `.filter()` that matched nothing) admitted every connection including anonymous ones; it now denies, matching `any()`, which was already correct by vacuous falsity.

- **Differential-privacy noise is no longer removable by a subscriber.** The noise seed was a public hash of public inputs, so any subscriber recomputed the draws and subtracted them, recovering the exact aggregate from one observation. Noise is now drawn from a counter-mode `HMAC-SHA256` stream keyed by an operator secret (`realtime({ privacySecret })`), shared across replicas so they still agree. The uniforms come straight off the MAC rather than seeding a 32-bit PRNG - a 32-bit seed would leave the whole per-window noise vector one of 2^32 candidates, and for integer-valued fields the published fraction pins the draw. The documented within-window limitation is unchanged: the offset is constant for a window, so an observer still sees exact deltas between updates inside one, and for a single-state or `lifetime` aggregate that window is the process lifetime.

- **Short-code keys widened from 32 bits to a 256-bit HMAC key.** The Feistel key was a 32-bit value, so one known `(id, code)` pair recovered it by offline brute force in about 30 seconds, letting an attacker decode any code and mint valid ones. `rounds` now has a hard minimum of 4, because each round mixes only one 32-bit round key and a lower count collapses the effective key space regardless of secret strength.

- **Tenant isolation now covers every publish surface.** An unscoped (null-tenant) connection could reach a victim tenant's `@t/` wire channel through `ctx.publishThrottled`, `ctx.publishDebounced`, `ctx.throttle`, `ctx.debounce` and the `ctx.batch` list form, which passed the handler's topic through unprefixed and unguarded. All of them now apply the same namespace rule as `ctx.publish`. This also fixes the mirror-image bug: a tenant-scoped connection's deferred publishes were not prefixed into its own namespace. Two consequences worth knowing before you upgrade. The reserved-`__` guard now covers these surfaces on **every** app, single-tenant ones included, so code that reached a framework-internal topic through `ctx.publishThrottled` or `ctx.batch` (rather than through `platform.publish`) now throws where it previously published. And a `@t/`-prefixed *logical* topic is refused on any connection without a resolved tenant - both on the deferred publish surfaces and on stream subscribe, which returns `INVALID_REQUEST` "Reserved topic prefix" - and that includes every connection on an app with no tenant resolver. This is deliberately not conditional on tenancy being configured, so that switching tenancy on later does not silently re-interpret those topics. It is **not** a blanket reservation, though: `ctx.publish` on the no-tenancy fast path and the top-level `publish()` export both still accept a `@t/`-prefixed topic, so an app determined to use that prefix can still do so through them - and those call sites would start throwing the day a tenant resolver is added. Treat `@t/` as reserved. Tenant-scoped connections are unaffected - their logical topics are wrapped into their own namespace exactly as before.

- **The 64-level envelope depth cap now honours a custom `maxEnvelopeDepth` on the upload ingress.** Upload chunk-0 arg headers were checked against the built-in default rather than the value passed to `handleRpc`, so an app that raised or lowered the bound got a different limit on uploads than on the text and binary paths.

- **DevTools redaction now covers RPC args and results and volatile sends**, not just stream-event payloads - the RPC path is the canonical credential carrier (`login({ password })`). Error results keep their `name`, `message` and `code`, which a generic key walk drops when they are non-enumerable (on a real `Error`, `message` and `name` always are; `code` is an own enumerable property on this client's `RpcError`, so it survived either way). Stack traces are now dropped explicitly at **every** depth of an error result rather than by relying on non-enumerability: a real `Error` was already safe by accident, but an error-shaped plain object - a serialized or hand-built one, which the duck-type branch also matches - carries `stack` as an own *enumerable* property, and a nested `cause.stack` or a back-reference smuggled the trace in even when the top level was clean. The drop applies only to *rejections*, so a successful call that legitimately resolves to error-shaped data (an error-log row, say) keeps its `stack` field intact. Redaction key matching is now genuinely case-insensitive: only the payload key was being lowercased, never the key set, so a documented extension like `__devtools.redactKeys.add('paymentMethod')` silently never fired. Note that redaction is key-based and therefore cannot blank a bare string, so error *messages* are captured verbatim; keep secrets out of them.

- **The Vite dev server no longer serves the generated live-registry to the client graph**, which enumerated every registered RPC, stream, cron and webhook path and disclosed the developer machine's absolute filesystem path via the `/@fs/` rewrite.

### Changed

- **BREAKING (`shortCodes`): existing codes change.** The key derivation moved to HMAC-SHA256 over your `secret`, so the same secret now maps an id to a different code. Because `decode` is total over the code space an old link does not fail cleanly - it resolves to a *different* id. Validate the decoded id against your store (already the documented contract) or mint fresh codes and retire old links.

- **BREAKING (`live.aggregate({ privacy })`): a `perturb` or `hybrid` strategy now requires `realtime({ privacySecret })`** and fails at init rather than at the first noise draw, so a misconfigured `hybrid` aggregate cannot boot cleanly and start throwing later when its cohort first reaches `k`. `suppress` (k-anonymity only) needs no secret.

### Fixed

- **A transferred (detached) binary id no longer terminates the process.** `_scalarId` reads a `Buffer`/`Uint8Array` id as bytes by constructing a view over its backing buffer, and that construction THROWS once the buffer has been detached - which is what happens to an id transferred to a worker. The throw was not contained to the call: the unknown-path reply reads the identity from a site OUTSIDE the dispatch try/catch, on a path that is fired without a `.catch`, so it surfaced as an unhandled promise rejection - process termination under Node's default, a crash loop rather than one failed request. It now returns `null` like every other unusable id shape, demoting that connection to a guest identity. Binary ids are additionally capped at 64 bytes: the derived key is rebuilt per message and used as a Map key and topic fragment, so an uncapped id was megabytes of string churn per frame.

- **An app-supplied session object can no longer throw into dispatch.** `ctx.user` arrives verbatim from `ws.getUserData()`, so reading it - or any of the `id`/`user_id`/`userId` aliases - can hit a throwing getter or a Proxy trap. Guarding only the binary-id constructor left that door open, and the call site outside the dispatch `try`/`catch` turns an escaping throw into an unhandled rejection, i.e. process termination. The whole probe is now guarded and returns `null`, which reads as anonymous. Because that demotion is otherwise silent - and it widens `live.rateLimit` quotas to per-socket and drops the per-user scoping on idempotency keys, so two callers sharing a client-chosen key can collide in one slot - a **one-shot dev warning** now fires when an id is PRESENT but unusable. It stays silent for the documented empty-string sentinel and for a session with no id at all, and it is compiled out of production.

- **`live.access.owner()` now says so when it denies a session the authenticated guard accepts.** `owner(field)` reads the one field you name (default `id`), while `guard({ authenticated: true })` probes `id`, `user_id` and `userId` in turn - so a `{ user_id }`-shaped session passes the guard and is denied by `owner()`, which reads as a framework bug from the app side. Widening the probe would ADMIT connections that are denied today, which is an authorization change rather than a fix, so the behaviour is unchanged and a one-shot dev warning now names the alias it found and the two ways out. Dev-only, and silent for a genuinely anonymous connection, which has no alias to suggest.

- **Batched RPCs now appear in DevTools.** The `batch()` collection path returned before the instrumentation call and stored the raw `resolve`/`reject`, so nothing sent inside `batch()` ever reached `__devtools.pending` or the history ring - a blind spot over exactly the path bulk traffic uses. (A batch of one is still sent as a bare RPC frame, so it was always visible.)

- **`locks` now declares the `true` form it has always accepted.** The runtime treats a non-array as "enabled, no allowlist", but the type declared `string[]` and the README showed only the array form, so a JS app passing `true` was silently outside the typed surface. Widened to `string[] | true` and documented; prefer the array, which is what bounds the keys clients can create.

- **A throwing property accessor in an RPC result no longer hangs the caller.** DevTools capture runs inside the settle path, *before* the RPC's own `reject`, so anything that throws there leaves the caller's promise permanently unsettled - a hang, not a lost log line. Three reads could throw. The duck-type probe evaluated `typeof result.stack` on a non-`Error`, which is exactly the cross-realm or hand-built shape the branch exists to catch and exactly where `stack` is likely to be a getter; it now tests `stack` by PRESENCE and never invokes it. The error-branch extras walk and the generic redaction walk both read every own key; both are now guarded, and an unreadable value is captured as `[unreadable]`. A real `Error` was safe only by accident - `instanceof Error` short-circuited the probe - which is why the pre-existing test for this passed while the shape it was written for still hung.

- **A detached (transferred) binary RPC payload now rejects with a code instead of a bare `TypeError`.** Building the wire view over a transferred `ArrayBuffer` throws - hash a `File` in a worker, then upload it - and the throw escaped the promise executor with no `code`, breaking the documented binary-RPC error contract. The pending entry also survived with its 30s timer still armed, so the sweeper later wrote a fabricated `TIMEOUT` record for a call that was never sent. It now rejects `INVALID_REQUEST` and settles cleanly.

- **The DevTools pending map no longer leaks an entry when a binary RPC header is oversized.** `__binaryRpc` registers the call with `_devtoolsStart`, but the `header exceeds 65535 bytes` bail-out rejected through the raw promise-executor `reject` rather than the instrumented wrapper stored on the pending entry, and the `pending.delete(id)` beside it clears the RPC map, not `__devtools.pending`. The call - with its captured args - therefore stayed in the window-reachable pending map for the life of the page. The timeout branches beside it are the same shape and stay correct the same way, by calling `_devtoolsEnd` explicitly rather than by going through the wrapper. Relatedly, both RPC paths now call `_connect()` *before* registering with DevTools: `_connect()` can throw (it is not callable under SSR), and registering first stranded a pending entry that nothing could sweep, since no timer is armed until afterwards.

- **The `__devtools` volatile-drop counter no longer under-reports until a backpressure drop.** `fireAndForget` bumps one module-level counter for every dropped send, but only the backpressure branch mirrored it onto `__devtools.volatileDropped`, and it does so by assigning the WHOLE counter rather than incrementing. Offline drops were therefore invisible in the panel until the next backpressure drop, at which point the field jumped by every offline drop accumulated since. The offline branch now mirrors too; the field counts all drop reasons, as its documentation always said.

- **The shipped `__devtools` declaration now describes the whole runtime object.** It declared only `history`, `streams` and `pending`, so the redaction extension the README documents (`__devtools.redactKeys.add('paymentMethod')`) did not typecheck for a TypeScript consumer. `volatile`, `volatileDropped`, `redactKeys`, `paused` and `smooth` are now declared, `history` entries carry their `seq`, and the `streams` entry gained the five fields it was missing (`merge`, `lastEventTime`, `lastEvent`, `error`, `recentEvents`) - the last of which the library's own devtools panel reads, so the declared type contradicted in-repo usage. Both ring arrays are typed `| null` because they are fixed-size and pre-filled: code that indexes them directly has to filter the holes, which the old type silently allowed it to skip. The doc comments now also state what redaction cannot reach (bare error strings) and that `paused` gates per-stream payload capture only.

- **The shipped `UploadContext` type no longer fails to compile against its own base.** `UploadContext extends LiveContext` while redeclaring `signal` as an `AbortSignal`, which is a different concept from `LiveContext`'s point-to-point `signal(userId, event, data)` method - so the interface did not typecheck (TS2430) and consumers who check library types saw the whole upload surface as broken. It stayed invisible because `skipLibCheck: true` is the SvelteKit/TS default. The base member is now `Omit`ted rather than redeclared, which is also the honest description: the upload runtime assigns the `AbortSignal` onto the context, so the point-to-point method was already unreachable inside a `live.upload()` handler at runtime. No rename, no runtime change, and the upload docs now say so. `npm run check` gained a rung that typechecks every shipped declaration with `strict` on and `skipLibCheck` off, so a broken `.d.ts` fails in-repo instead of only for consumers.

- **A throwing watcher no longer silently kills the whole reactive layer.** The recursion-guard depth counter was incremented and decremented without `try`/`finally`, so a throw from any watcher leaked it; after nine leaks the guard was permanently tripped and every derived stream, effect, flag cell, aggregate and webhook fan-out stopped firing for the rest of the process lifetime. Windowed aggregates additionally re-arm their boundary and slide timers in a `finally`, so a failed window publish is reported and retried on the next boundary instead of stopping the window forever.

- **`live.smooth` re-syncs no longer serve stale entity state.** The sync roster is built fresh per call; a remembered roster handed back positions the authority had already replaced, and tick deltas could not repair it because a tick only carries entities that changed that tick - so an entity that moved and then stopped was never re-sent to a reconnecting client. Flooding is bounded by a per-connection re-sync allowance instead, which also caps the reply serialization a cache could not.

- **`live.smooth` centre reports no longer force a full relevancy pass on sub-threshold jitter.** Only a subscriber may steer relevancy, and a report must move the centre beyond a fraction of the interest radius (measured from the centre the last pass ran at, so a slow drift still accumulates into one) before it owes an `O(subscribers x entities)` recompute.

- **Multiplayer sticky presence fields are bounded consistently across single-node and cluster.** Both the in-memory and Lua merge paths now measure the entry cap in BYTES; the JS side counted UTF-16 code units, so a value built from multi-byte characters let the default no-Redis path admit roughly four times what the cluster path admitted. Undeclared lock keys, oversized values and over-cap merges now emit a one-time dev warning instead of being silently dropped.

- **The generated `$types.d.ts` quotes every interpolated string literal**, so a crafted filename cannot break out of a `declare module` name and inject ambient declarations.



- **Dev server: live functions were "not registered" on the very first `npm run dev`.** On a cold start (empty `node_modules/.vite`), Vite runs its first-pass dependency optimization and rebuilds the SSR module graph while the page loads. That teardown could split `svelte-realtime/server` into two module instances: the one `svelte-adapter-uws` bound its `message` handler to, and a second one the plugin's one-shot registry load (fired on the server's `listening` event) populated. The handler then read an empty registry, so every RPC and stream failed with `RpcError: Not found` / `[svelte-realtime] RPC call to '...' - no such live function registered`, and the page sat on "Connecting...". It self-healed only after a restart with a warm cache, so a brand-new user following the getting-started counter hit it on their first run while a warm-cache test suite never did. The Vite plugin now imports the generated registry directly from your `src/hooks.ws.*` module in dev, making the registry a dependency of the exact module graph the handler is bound to - the cold and warm paths are now identical. The injection is SSR-only, root-scoped, and idempotent; production builds are unaffected (packaging already injects the registry through the SSR input).

- **Removed a false "does not export a `message` handler" warning for the scaffold's own hooks form.** The scaffold and the e2e fixture write `import { message } from 'svelte-realtime/server';` followed by a bare `export { message };` (a re-export with no `from` clause). The hooks check only recognised `export { message } from '...'` and `export const/function message`, so it warned "WebSocket RPC calls will go unhandled" on a file that was, in fact, wired correctly. The check now recognises any `export { ... message ... }` specifier list, and probes `hooks.ws.mjs` in addition to `.js` / `.ts` for parity with the adapter's handler discovery.
...
- **A room join batch whose data-stream subscribe is DENIED no longer hangs the entire batch (and no longer leaks an in-flight request that stalls graceful shutdown).** A room's data-stream subscribe opens a per-socket claim barrier at dispatch topic-resolution - the owner first-joiner delivery, and now the presence self-delivery - which the paired `:owner` / `:presence` loader in the same parallel batch awaits. If the data subscribe was then refused (a per-topic subscribe hook that authorizes viewing the sub-stream but denies the data topic, or the `MAX_SUBSCRIPTIONS_PER_CONNECTION` boundary where the parallel sub-stream subscribe wins the last slot), the early return skipped the barrier settle, so the sub-stream loader awaited a resolution that never arrived: the batch's `Promise.all` never settled, the client got NO response for the whole batch, and the hung request's in-flight count leaked - deferring `onShutdown`'s drain indefinitely. Every early exit after the barrier opens (a stream-filter or class-of-service rejection, a subscribe throw, a subscribe denial) now settles both barriers to null, so the loader reads the shared store and the batch always responds. A guarded room's guard denies by THROWING (not returning a code), which escapes to the caller's error handler with the barrier still open; that path is settled too - so a refused guarded join cannot strand the barrier, which additionally closes a latent hang where a LATER lone `:presence` / `:owner` subscribe on the same socket awaited a barrier the failed join left open (the join never subscribed, so no unsubscribe drain ever cleared it). The owner-barrier variant of these hangs has existed since the owner first-joiner delivery shipped; all are fixed together.
- **A room member now always sees ITSELF in the presence roster - on a cluster, every joiner previously saw everyone except themselves.** The client batches a room's data-stream and `:presence` subscribes into one wire frame, and the batch executes its items in parallel: the `:presence` loader's shared-roster read could beat the data-join's cluster acquire (an `owner: true` room delays the acquire further behind the owner join's redis round-trips, making the loss deterministic there), and the acquire's live `join` reached the socket before the client had registered the `:presence` store, so it was dropped. Net effect with `platform.redis` wired: the joiner's own entry was in neither the snapshot nor the live stream - each member's roster showed only the OTHER members (the demo-lobbies "each viewer undercounts" report). The single-process path was immune (the in-memory ref is written before the race window), which is why it never surfaced outside a cluster. The joiner's own roster entry is now sequenced into the `:presence` subscribe response itself via the same per-socket claim barrier the owner first-joiner delivery uses: dispatch opens it before any loader in the batch runs, the data-join resolves it the moment the presence payload exists (before the owner join), and the `:presence` loader merges it into the snapshot. A lone `:presence` viewer with no paired data-join is never injected; `live.multiplayer` inherits the fix (it reuses the room presence stream); the in-memory path is unchanged. Barrier slots are scoped to the membership: the data-stream unsubscribe drain clears the socket's claim slots (presence AND owner), so a join that never paired with a sub-stream subscribe cannot serve a stale value to a later lone `:presence` or `:owner` subscribe on the same socket - the owner variant of that staleness existed since the owner first-joiner delivery shipped and is fixed by the same clearing.
- **`live.forget` now announces the owner successions the durable store already committed even when the erasure is signalled incomplete for retry.** The cluster-wide owner eviction erases the user room by room; if one room's hand-off commits but a sibling room hits a transient error, the store rejects the whole purge (the erasure is incomplete and the app must retry) - but it carries the already-committed successions on the thrown error. Previously that rejection published nothing, and the retry finds the committed room already handed off (it reports no succession the second time), so a committed `u1 -> u2` change reached the `:owner` stream on NEITHER the failing call nor the retry - already-subscribed clients kept the erased owner label until the next owner transition. `live.forget` now publishes those committed successions from the error before surfacing `FORGET_STORE_FAILED`, so each reaches the wire exactly once; the announcement is best-effort and fully guarded, so a throwing bus can never mask the retry signal. Needs the producing half in `svelte-adapter-uws-extensions`' `createForgetStore` (>= the release that carries `ownerSuccessions` on the incomplete-erasure error); an older store that never partially fails is unaffected.
- **Dropping a stream's last subscriber now releases the server-side subscription while the socket stays open - room registry counts, presence rosters, and owner succession no longer wait for the socket to close.** The adapter client (0.6.0-next.61+) sends no wire unsubscribe frame for a server-managed topic, reasoning it never sent the subscribe frame - but the server holds real wire state for it (the stream RPC ran `platform.subscribe`), and only the client knows when the last local ref let go. So a "leave" with the tab still open told the server nothing: the adapter kept delivering publishes to the socket, the app's `unsubscribe` hook chain never fired, and every unsubscribe drain - the room enumeration registry release, the presence `leave`, the owner succession - deferred to socket close. On a lobby this is the "member count climbs while one user cycles join/leave" leak, and it matches the production `subs.total-mismatch` adapter assert. The realtime client now refcounts the managed topics it marks and, when the last local stream ref releases one on an open socket, sends the explicit unsubscribe frame itself. Duplicate-safe against a future adapter-side fix (the server handles a redundant unsubscribe as a no-op), refcounted so two stream instances sharing one wire topic never cut each other off, and a no-op on older adapters without managed-topic marking (their own topic refcount already sends the frame). The frame is handed to the adapter's own send (gated on the socket being OPEN), so it still fires on a backgrounded (`'suspended'`) tab whose socket is live - the exact "unmount live views on hide" pattern that would otherwise re-leak - and is skipped only when the client is `terminated` by an explicit `connect().close()`, where there is no subscription to release and re-reading connection status would wrongly reconnect the closed socket. Reminder that a custom `unsubscribe` hook in `hooks.ws.js` must chain the framework's - now documented in the README.

## [0.6.0-next.87] - 2026-07-16

### Fixed

- **A single-process `live.flag().set()` (and any out-of-band `publish()` to a flag's default replay topic) no longer prints a spurious "install the replay extension" warning.** The previous `[Unreleased]` fix routed the top-level `publish()` - and therefore `flag.set()` - through the replay buffer. A flag enables a single-entry replay buffer implicitly by default, so in a single-process app with no `platform.replay` that routing tripped the missing-extension dev warning on every flag's first `.set()` - advice that is wrong for a flag (the correct opt-out is `replay: false`, not installing an extension) and that contradicts `live.flag`'s documented "single-process apps lose nothing". The framework now marks a flag's DEFAULT buffer as an implicit opt-in and suppresses the missing-extension warning for it, exactly as the room-owner `:owner` default-replay topic is already suppressed; the buffer still engages normally when the replay extension is present. A flag declared with an EXPLICIT `replay` option is a real opt-in, so a missing extension there still warns. Only the dev warning changed - the wire behavior (bare `platform.publish` when no replay, buffer routing when the extension is present) is unchanged.
- **The outbound-webhook dead-letter forget tombstone now uses monotonic time, so a backward wall-clock step cannot resurrect an erased user's in-flight delivery.** The tombstone match (purge time vs delivery start time) read the ~1s-cached wall clock: a backward NTP correction between a delivery's start and a `live.forget` could make `purgeTime < startedAt` and RETAIN the forgotten user's payload past a confirmed erasure, and within one cached wall tick a purge and a genuinely-later new delivery could stamp identically and over-drop the new record. Both the purge stamp (`purgeUser`) and the delivery-start stamp (`_fireWebhookOut`) now read the runtime monotonic clock (step-immune, sub-ms), and the tombstone sweep matches; the operator-facing `failedAt` / TTL stay on the wall clock. A caller that does not thread `startedAt` falls back to the current monotonic time (never the wall `failedAt`), so the two are never compared across clock domains.
- **`live.forget` now publishes owner successions the durable store resolves cluster-wide, so an erased owner no longer survives on instances the forget never touched.** The remaining half of the cross-instance owner-succession gap: forget's succession trigger is double-gated on the forgetting instance's LOCAL presence ref and LOCAL room membership, so a forget running on an instance where the erased user never connected (a load-balanced forget request, a dedicated erasure job) triggered no succession at all - and even a locally-triggered leave only runs succession when the identity's cluster-wide connection count reaches zero, so a user still connected on another instance stayed owner-of-record past a confirmed erasure. Both need cluster-wide resolution, which lives with the durable forget store (the single authoritative cross-instance forget actor). The store seam now accepts a third `purgeUser` return shape - the envelope `{ rowsAffected, ownerSuccessions }` - where `ownerSuccessions` lists the `:owner` changes the store's cluster-wide owner eviction produced (`{ topic, owner, reason }` per room). `live.forget` publishes each on the room's `:owner` topic through the replay buffer (the same path a local forget succession uses), so live subscribers on every replica see the successor and a resuming or fresh client gap-fills it instead of the erased owner; the count surfaces as `surfaces.ownerSuccessions`. Plain count returns (every existing store) are byte-identical, single-instance apps are unchanged, and the room-level `onOwnerChange` hook deliberately does not fire for fan-in successions (it is bound to local join state the forgetting instance need not hold). The producing half - the durable store's owner-hash force-evict with a user-to-owned-rooms reverse index - ships in `svelte-adapter-uws-extensions`' `createForgetStore`.
- **The idempotency store's forget tombstone now uses monotonic time with an explicit grace window, so a backward wall-clock step cannot let an erased user's in-flight request commit and cache PII for the full TTL.** Same NTP-backward-step hole the dead-letter tombstone just closed, in the store that guards `live.idempotent`: a request acquired its slot at wall `W1`, a backward NTP step, then `live.forget(user)` stamped the tombstone at wall `W2 < W1`, so the racing commit's `tomb >= acquiredAt` test was FALSE and the forgotten user's freshly-computed result cached for the full TTL (a residual-PII window a purge that scans only already-committed keys never sees). The acquire stamp and the purge stamp now read the runtime monotonic clock (step-immune, sub-ms); the cached result's `expiresAt` and the sweep throttle stay on the wall clock. Because the old drop test leaned on the coarse 1Hz wall clock to conservatively over-drop a duplicate that re-acquired within the same tick as the purge, the monotonic test keeps that grace explicitly (`tomb + 1000ms >= acquiredAt`) rather than dropping it - an in-flight request is still dropped, a duplicate within 1s of the purge is still dropped, and a genuinely new request past the window caches again. Single-process and cluster behavior are otherwise unchanged; durable idempotency stores need the same monotonic tombstone.
- **A `piiRedact` factory (dynamic-topic) stream whose topic cannot be reduced to a safe cluster pattern now dev-warns instead of silently having no cluster-transient redaction coverage.** next.86 added send-side redaction for a resolved factory topic an instance never subscribed to, keyed by the factory's derived skeleton - but a topic read from `ctx` (the idiomatic multi-tenant `(ctx, room) => ctx.user.org + ':' + room`), a variable-first topic (`(ctx, id, kind) => id + ':' + kind`), or a constant compiles to no pattern and was skipped SILENTLY. So a developer's PII could relay raw on a never-subscribed cluster instance with no signal that their `piiRedact` was cluster-incomplete. Declaring such a stream now emits a one-shot dev warning naming the derived skeleton and the required shape (a leading literal namespace plus single-segment args, e.g. `chat:` + roomId). The README `piiRedact` section documents the cluster behavior and the narrow composite-id residual (a single arg that itself contains a `/` or `:`). Production stays silent; single-instance deployments and any topic a local subscriber has resolved are unaffected.
- **`live.forget` owner succession now advances the `:owner` replay buffer on any instance that runs the purge, not only one that locally subscribed `:owner`.** next.86 published the succession live and wrote the buffer, but only when the purge instance had locally registered the `:owner` topic as replay-eligible (a local claim or a local `:owner` subscribe). A forget that runs succession on an instance which never held a local `:owner` subscriber - a load-balanced forget request, or a dedicated erasure job - advanced no buffer, so a resuming or fresh client gap-filled the erased owner from the stale buffer instead of the successor (a privacy leak that survived the next.86 fix in the cross-instance case). The `:owner` sub-stream is replay-backed by construction, so the purge now registers it as replay-eligible whenever the replay extension is present (idempotent, mirroring the claim-time registration), while `_maybeReplayPublish` still owns the `WRAPPED_FOR_REPLAY` defer and the sync-throw fallback. Single-process apps (no `platform.replay`) are unchanged. Note: a forget that runs on an instance where the erased user has no local presence ref at all still resolves succession only from local membership - fanning forget across the cluster is a separate, broader concern.
- **`live.smooth` `hitTest.onHit` now fires - the server codegen registers the `__smooth/shoot` RPC.** The client always sends a shot to `<module>/<name>/__smooth/shoot` (the generated `view.shoot()` seam) and the server defines the `__smoothShoot` handler, but the Vite server-registry codegen registered only `command`, `sync`, and `center` - never `shoot` - so every shot hit an unregistered path and was silently dropped: `onHit` never ran, hits resolved to zero, even on a single instance. (Unit tests hand-registered the shoot handler, so they stayed green while the real wire path was dead.) The registry codegen now emits the `__smooth/shoot` registration alongside the others - one line that repairs the production build, dev startup, and hot reload, which all load the generated registry - and the direct-load fallback path (used only when the generated registry fails to load) is brought to parity, since it was additionally missing `center` as well as `shoot`.
- **The top-level `publish()` export (and `live.flag().set()`, which uses it) now applies a topic's `transform` before the replay buffer, matching `ctx.publish`.** next.86 routed `publish()` through the replay buffer and applied `piiRedact`, but it skipped the per-event `transform` step. So an out-of-band publish to a `{ replay, transform }` topic wrote the RAW value to the buffer while a live subscriber (served by `ctx.publish`) saw the projected shape - a resuming subscriber then gap-filled a frame inconsistent with the live stream, and any un-projected field the transform was meant to drop (e.g. an internal `secret`) rested in the replay store. `publish()` now applies the subscribe-registered `_topicTransform` for the topic before redaction and before the buffer write, in the same order `ctx.publish` uses (transform, then redact, then replay); a throwing transform routes to the stream's `onError` and drops the publish, or propagates when no observer is set. Only the subscribe-time transform registry carries transforms (there is no declaration-time transform map), so a topic with no active subscriber on the publishing instance is unchanged, as is any topic without a transform.

## [0.6.0-next.86] - 2026-07-13

### Fixed

- **`live.forget` now notifies subscribers when erasing a user who owned a room, so the successor is delivered instead of the erased owner lingering.** Purging a user runs owner succession (a new owner is chosen), but on the forget path the `:owner` change was published through a no-op, so connected `room.owner` stores kept showing the erased user indefinitely and a resuming client gap-filled the stale owner from the replay buffer (which was never written) - a stale-owner correctness bug and a privacy leak, and unlike a presence leave it does not self-heal on the forgotten user's later disconnect (its presence ref is already gone). The succession is now published through the platform, so live subscribers always see the successor; it also writes the `:owner` replay buffer when that topic's buffer is active on the instance running the purge (the normal case, once any client has subscribed to `:owner`), so a resuming client reads the successor too. Single-process apps and the normal (non-forget) succession paths are unchanged.
- **`live.forget` no longer leaves an erased user's data in the outbound-webhook dead-letter queue when a delivery was in flight during the purge.** `live.forget` purges the DLQ once, at purge time. But an outbound webhook delivery is fire-and-forget and long-lived (retries with backoff), and the record is written only when delivery finally fails - so a delivery that began before `forget(user)` and exhausted after it re-inserted that user's payload into the DLQ after `forget` had already returned success, where a later replay could re-fire the erased user's event. The dead-letter store now carries a forget tombstone (mirroring the idempotency store's racing-commit defense): `purgeUser` records the purge time, `_fireWebhookOut` threads the delivery start time into the store, and a capture whose delivery started at or before a purge is dropped instead of retained. A genuinely new delivery started after the purge, and other users' deliveries, are unaffected. A single delivery whose total duration exceeds the tombstone window (10 minutes, far above a default retry budget) is a narrow documented residual, and records captured without a `forgetUserId` extractor remain unattributable (the documented pre-existing limitation). Durable (cluster) dead-letter stores need the same tombstone in their `add`/`purgeUser`.
- **A cluster `live.alarm` armed on a non-leader instance now fires on time instead of up to `pollMs` late (or being silently dropped).** With a durable store and a leader configured, the precise in-memory timer was gated on leadership, so an alarm armed on a non-leader (the majority, since clients are load-balanced) never fired from its own timer - it fell to the leader's periodic recovery sweep (default 15s cadence), and any `misfireMs` smaller than that sweep interval dropped it entirely, in a perfectly healthy cluster. The store's atomic `delete`-claim already guarantees single-fire across every instance, so the leader gate on the precise path was redundant; it now applies only when there is no store to arbitrate. With a store, the arming instance fires precisely and the claim still prevents any double-fire with the leader's recovery sweep. The discouraged leader-without-store config is unchanged (leader-only firing), as is single-instance.
- **The top-level `publish()` export (and `live.flag().set()`, which uses it) now routes replay-eligible topics through the shared replay buffer.** Every other publish seam - `ctx.publish`, cron auto-publish - routes replay, but the out-of-band `publish()` applied redaction and then called the adapter's bare `publish`, so a publish to a `replay: true` topic broadcast live but never wrote the buffer and carried no authoritative seq. The sharpest case was `live.flag`: a flag defaults to a single-entry replay buffer, and `.set()` goes through this export - so a replica that booted after a `.set()`, or a subscriber offline during it, served the stale init value indefinitely, defeating the buffer's whole purpose. `publish()` now routes through the same replay path as `ctx.publish` (a jittered control event keeps the direct path; a topic with no replay registration is unchanged), so an out-of-band publish and a `flag.set()` land in the buffer with a seq and reach a late replica or a resuming subscriber. Single-process apps (no `platform.replay`) are unchanged.

- **`piiRedact` now covers a dynamic (factory) topic published from an instance that never subscribed to it.** Redaction runs on the publishing instance before the wire, so it needs that topic's redactor in hand. A static topic registers its redactor at declaration; a resolved dynamic topic registered one only when a local subscriber resolved it. In a cluster an instance can publish to a resolved dynamic topic it never subscribed to (a peer holds the subscriber) - it then had no redactor and relayed a RAW frame to the peer, which delivered it un-redacted, since the receiving side never re-redacts. A factory topic's redactor is now also registered at declaration time keyed by its derived pattern (`chat/{arg0}`), so a publish to any resolved instance of that topic redacts send-side, before the buffer and the cluster relay - closing the transient dynamic-topic gap without a cross-instance relay protocol. Only a topic with a leading literal prefix registers (`room:{id}`; a variable-first pattern like `{id}:{kind}` is skipped since it would match nearly every topic), and each variable matches one `/`- or `:`-delimited segment. A pattern match is a heuristic guess that a resolved topic belongs to the factory, so two guards keep it off co-located streams: a topic another stream or channel declared explicitly (a static `chat/typing` beside a `chat/{room}` factory) is never pattern-matched, and the match is fail-open - if the redactor throws on a matched topic, the publish passes through raw instead of being dropped (raw is exactly the pre-fix cluster-transient behavior, never worse). The narrow residuals: an undeclared ad-hoc publish to a topic inside the pattern's namespace under a fields redactor (that field is omitted - the message is still delivered); a co-located sibling written as a rare constant-returning factory rather than a static string (a topic factory is never invoked at declaration, so declare co-located topics as strings for ownership protection); and two distinct factory streams that derive the same skeleton (give them distinct topic shapes; dev-warned). Topics outside a registered pattern are untouched; static, tenant-static, and single-instance deployments are byte-identical.
- **`live.idempotent` no longer double-applies an effect when the commit fails after the handler ran.** The handler and the idempotency-record commit shared one `catch` that called `slot.abort()` on any throw. So a durable store that applied its record and then lost the acknowledgement (or rejected after a partial write) had that committed record DELETED, and a retry with the same key re-ran the handler and applied the effect twice. The handler and the commit now have separate error scopes: a pre-commit handler failure aborts the slot (a legitimate retry re-runs), while a commit failure leaves the slot intact and surfaces a typed `LiveError('IDEMPOTENCY_COMMIT_UNKNOWN')` so the caller reconciles rather than blindly retrying. The in-memory store's commit does not throw, so single-process apps are unchanged.
...
- **Deterministic-netcode simulation: `svelte-realtime/sim` now exports `runSmoothSim`, `replaySmoothSim`, and `runSmoothSimSwarm` for the server-rewind lag compensation.** A seeded, latency-varying shot stream resolved against a moving board reproduces every hit - target, distance, rewind instant, impact point - bit for bit under the same seed; a divergence between two passes is a real determinism bug. The swarm runs thousands of randomized worlds and reports `{ summary, runs }` in the same shape as the other sims (`runLiveSimSwarm` / `runSimSwarm`), so a CI sweep or a status board can assert "N seeds, 0 violations". It drives the real resolution path - the reach measurement, the favor-shooter clamp, the candidate gate, the rewound narrowphase, the nearest-first `onHit` - over the real history ring, monotonic clock, interest relevancy, and round-trip tracker, so nothing is re-implemented. `faultMode` widens the shot lag past the reach and teleports a target into the rewindable window, exercising the reach clamp, the over-window fallback, and the ring's teleport guard. Per-shot domain invariants (bounded reach, no future or out-of-window rewind, a real `[0,1]` ray fraction) are checked inline; `checkRatio` re-runs a fraction of seeds for the two-pass determinism gate.
